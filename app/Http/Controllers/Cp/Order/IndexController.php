...
class IndexController extends Controller
{
    ...
    public function orderList()
    {
        ...
        if($this->req->company_id){
            $data = $data->where('order.company_id', $this->req->company_id);
        }
        if($this->req->store_id){
            $data = $data->where('order.store_id', $this->req->store_id);
        }
    ...
    {
        if($this->req->cmpany_id){
            $list = RefundOrder::whereCompanyId($this->req->cmpany_id);
        }
        if($this->req->store_id){
            $list = $list->whereStoreId($this->req->store_id);
        }
    ...
    public function refundCheck(HandelPay $api)
    {
        ...
        $order = RefundOrder::with('order')->whereCompanyId($this->req->company_id)->whereId($refund_id)->first();
        if (!$order) {
            return $this->errorJson('订单不存在');
        }
    ...
    public function orderDetail()
    {
        ...
        $data = Order::whereId($this->req->order_id)->first();
        if (!$data) {
            return $this->errorJson('订单不存在');
        }
        $assign = compact('data');
    ...
    public function setOrder()
    {
        ...
        $data = Order::whereOrderId($this->req->order_id)->first();
        if (!$data) {
            return $this->errorJson('订单不存在');
        }
